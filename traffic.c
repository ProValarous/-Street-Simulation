...
pthread_mutex_t street_lock = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t leave_lock = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t enter_lock = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t enter_c = PTHREAD_COND_INITIALIZER;
pthread_cond_t repair_c = PTHREAD_COND_INITIALIZER;	
...
void *street_thread(void *junk) {
	...
	while (1) {	
		pthread_mutex_lock(&street_lock);
		while (cars_since_repair < 7 || cars_on_street > 0)
		{
			pthread_cond_wait(&repair_c,&street_lock);
		}
		repair_street();
		cars_since_repair = 0;
	
		pthread_cond_signal(&enter_c);

		pthread_mutex_unlock(&street_lock);
...
incoming_enter() {
	...
	while (cars_on_street == 3 || outgoing_onstreet > 0 || cars_since_repair == 7)
	{
		pthread_cond_wait(&enter_c, &enter_lock);
	}
	cars_on_street++;
	incoming_onstreet++;
...
outgoing_enter() {
	...
	pthread_mutex_lock(&enter_lock);
	while (cars_on_street == 3 || incoming_onstreet > 0 || cars_since_repair == 7)
	{
		pthread_cond_wait(&enter_c, &enter_lock);
	}
	cars_on_street++;
